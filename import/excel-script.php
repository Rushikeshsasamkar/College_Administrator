    ...
    if ($arrFileName[1] == 'csv') {
        ...
        while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
            ...
            if ($count == 1) {
                ...
                $hobby_drawing = $connection->real_escape_string($data[16]);
				$hobby_singing = $connection->real_escape_string($data[17]);
                $hobby_dancing = $connection->real_escape_string($data[18]);
				$hobby_cooking = $connection->real_escape_string($data[19]);
                $hobby_other = $connection->real_escape_string($data[20]);
				$other_hobby = $connection->real_escape_string($data[21]);
